...
tests_secc_DIR = tests/secc
tests_secc_SOURCES = \
	$(tests_secc_DIR)/tests.c
tests_secc_OBJECTS = $(tests_secc_SOURCES:.c=.o)
...
test: dev tests-all-build antispoof core diode secc rxtx pmtud ip-fragment firewall nic print queue switch vtep tap thread integration vhost
	@echo "tests ended"

tests_compile: dev tests-antispoof tests-core tests-diode tests-secc tests-rxtx tests-pmtud tests-ip-fragment tests-integration tests-nic tests-print tests-queue tests-switch tests-vhost tests-vtep tests-tap tests-thread tests-firewall
...
tests-secc: dev $(tests_secc_OBJECTS)
	$(CC) $(tests_CFLAGS) $(PG_ASAN_CFLAGS) $(tests_secc_OBJECTS) $(tests_LIBS) -o $@

$(tests_secc_OBJECTS): %.o : %.c
	$(CC) -c $(tests_CFLAGS) $(PG_ASAN_CFLAGS) $< -o $@
